document.addEventListener('DOMContentLoaded', function() {
    ...
    function createCard(currency, type) {
        ...
        let cardHTML = `
            <h3 class="card-title" style="color: ${titleColor}">${currency.name}</h3>
        ...
        if (currency.buy !== null) {
            cardHTML += `
                <div class="card-value">
                    <span>Compra:</span>
                    <span class="buy-value">$${currency.buy.toFixed(2)}</span>
                </div>
            `;
        }
        ...
        if (currency.sell !== null) {
            cardHTML += `
                <div class="card-value">
                    <span>Venta:</span>
                    <span class="sell-value">$${currency.sell.toFixed(2)}</span>
                </div>
            `;
        }
        ...
        if (currency.spread !== null) {
            cardHTML += `
                <div class="card-spread">
                    <span>Spread:</span>
                    <span>$${currency.spread.toFixed(2)}</span>
                </div>
            `;
        }
        ...
        cardHTML += `
            ...
            <button class="card-btn" data-currency="${encodeURIComponent(JSON.stringify(currency))}">Calcular</button>
        `;
        ...
        calculateBtn.addEventListener('click', function() {
            ...
            calculateSingle(currencyData, card);
        });
    ...
    function calculateAll() {
        ...
        currencyData.steam.forEach(currency => {
            ...
            cards.forEach(card => {
                if (card.querySelector('.card-title').textContent === currency.name) {
                    if (currency.buy !== null) {
