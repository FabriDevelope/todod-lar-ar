document.addEventListener('DOMContentLoaded', function() {
    ...
    function renameCurrency(d) {
        if(d.casa) {
            ...
            if(name.includes('oficial')) return d.casa;
    ...
    // 🔹 Intervalo de actualización y barra
    let updateInterval = 60000; // 1 min
    const bar = document.getElementById('update-bar');
    const updateText = document.getElementById('update-text');
    const intervalSelect = document.getElementById('update-interval-select');
    let updateTimer, barTimer;
    ...
    intervalSelect.addEventListener('change', ()=>{
        ...
        const minutes = Math.floor(updateInterval / 60000) || 1;
        updateText.textContent = `Actualización cada ${minutes} minuto${minutes>1?'s':''}`;
    ...
    // Inicialización
    updateText.textContent = 'Actualización cada 1 minuto';
    fetchCurrencyData();
    updateTimer = setInterval(fetchCurrencyData, updateInterval);
});
