document.addEventListener('DOMContentLoaded', function() {
    ...
    function renameCurrency(d) {
        if(d.casa) {
            ...
            if(name.includes('oficial')) return d.casa; // Mantener el nombre de la API con país
            if(name.includes('euro')) return d.casa; 
    ...
    function createCard(currency, type) {
        ...
        // 🔹 Abrir modal de calculadora específica
    ...
    function openCalcModal(currency){
        ...
        // Event listeners temporales para cada apertura
    ...
    document.getElementById('currency-input').addEventListener('keypress', e=>{if(e.key==='Enter') document.getElementById('calculate-all-btn').click();});
    ...
    const updateInterval = 60000; // 60 segundos
let updateProgress = 0;

function startUpdateBar() {
    const bar = document.getElementById('update-bar');
    const stepTime = 100; // cada 0.1 seg
    const stepWidth = 100 / (updateInterval / stepTime);

    if(window.updateBarTimer) clearInterval(window.updateBarTimer);

    updateProgress = 0;
    bar.style.width = '0%';

    window.updateBarTimer = setInterval(() => {
        updateProgress += stepWidth;
        if(updateProgress >= 100) updateProgress = 100;
        bar.style.width = `${updateProgress}%`;
    }, stepTime);
}

// 🔹 Llamamos al inicio y cada vez que fetchCurrencyData corre
startUpdateBar();
setInterval(() => {
    fetchCurrencyData();
    startUpdateBar();
